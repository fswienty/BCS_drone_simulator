...
class Drone:
    ...
    def __init__(self, manager, name: str, position: Vec3, uri="-1", printDebugInfo=False):
        ...
        # self.ghost.addShape(BulletSphereShape(self.SENSORRANGE / 2))
        ...
        for node in self.ghost.getOverlappingNodes():
            if node.name.startswith("drone"):
                ...
                if distVec.length() < 0.2:
                    print("BONK")
                distMult = max([0, self.SENSORRANGE - distVec.length()])  # make this stuff better
                distMult = distMult
    ...
    def _updateAvoidanceForce(self):
        ...
        allDrones = self.manager.drones
        for drone in allDrones.values():
